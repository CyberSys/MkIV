...
void ReliabilityLayer::InitializeVariables( void )
{
	...
	lastTimeBetweenPacketsIncrease=lastUpdateTime;
	...
	lowestPing=(RakNetTimeNS)1000000; // 1000 ms to start with
	ackTimeIncrement=(RakNetTimeNS)1000000; // 1000 ms to start with

	nextSendTime=(RakNetTimeNS)0;
...
bool ReliabilityLayer::HandleSocketReceiveFromConnectedPlayer( const char *buffer, int length, SystemAddress systemAddress, DataStructures::List<PluginInterface*> &messageHandlerList, int MTUSize )
{
	...
	if (hasAcks)
	{
		...
		RakNetTimeNS ackPing = time - rst;
		SetPing((RakNetTime)(ackPing/(RakNetTimeNS)1000));
		if (continuousSend)
		{
			if (ackPing < lowestPing*(RakNetTimeNS)2 + (RakNetTimeNS) 1000 ) // 1 milliseconds tolerance, because at small pings percentage fluctuations vary widely naturally
			{
		...
		//		printf("- CurPing: %i. LowestPing: %i. Ploss=%.1f.  Time between= %i\n", curPing, (RakNetTime)(lowestPing/(RakNetTimeNS)1000), 100.0f * ( float ) statistics.messagesTotalBitsResent / ( float ) statistics.totalBitsSent, timeBetweenPackets);
			}
			else if (ackPing >= lowestPing*(RakNetTimeNS)2 + (RakNetTimeNS) 1000) // 1 milliseconds tolerance, because at small pings percentage fluctuations vary widely naturally
			{
...
#ifdef _DEBUG
	//			printf("over_Buffered_holeCount=%i\n", holeCount);
#endif
				// Security against buffer overflow
				if (holeCount - hasReceivedPacketQueue.Size() > 1000)
				{
					// Don't ack - if this was a real message that is that far out of order, just let it be resent because we're not going
					// to track it.
					delete [] internalPacket->data;
					internalPacketPool.Release( internalPacket );
					goto CONTINUE_SOCKET_DATA_PARSE_LOOP;
...
void ReliabilityLayer::Update( SOCKET s, SystemAddress systemAddress, int MTUSize, RakNetTimeNS time, DataStructures::List<PluginInterface*> &messageHandlerList )
{
	...
	if (time >= nextSendTime)
	{
		...
		if (availableTime > timeBetweenPackets*10)
			availableTime=timeBetweenPackets*10;
		...
		while ( usedTime < availableTime )
		{
			...
			if ( GenerateDatagram( &updateBitStream, MTUSize, &reliableDataSent, time, systemAddress, messageHandlerList ) )
			{
				...
				nextSendTime=time+timeBetweenPackets;
...
void ReliabilityLayer::SetPing( RakNetTime i )
{
	...
	if (i > timeoutTime)
		...
		curPing = i;
	//	if (curPing < 30)
	//		curPing=30; // Leave a buffer for variations in ping
#ifndef _RELEASE
	if (curPing < (RakNetTime)(minExtraPing+extraPingVariance)*2)
		curPing=(minExtraPing+extraPingVariance)*2;
#endif
...
void ReliabilityLayer::UpdateNextActionTime(void)
{
	...
	if (curPing*(RakNetTime)PING_MULTIPLIER_TO_RESEND < MIN_PING_TO_RESEND)
		ackTimeIncrement=(RakNetTimeNS)MIN_PING_TO_RESEND*1000;
	else
		ackTimeIncrement=(RakNetTimeNS)(curPing*(RakNetTime)PING_MULTIPLIER_TO_RESEND)*1000;
}
